# Extension Adapter Dashboard Content Sub Item

[← Back to Content Sub Items](index.md)

A **Extension Adapter Dashboard** Content Sub Item is used to render a Relational View in an Extension Adapter Dashboard experience within a Content Item layout.

Extension Adapter Dashboard Content Sub Items allow you to tailor the presentation and behavior of a Relational View for a specific page. This means the same Relational View can be reused across multiple Content Sub Items and configured differently depending on the user experience you want to create.

## Overview

Use an Extension Adapter Dashboard Content Sub Item to:

- render a Relational View in an Extension Adapter Dashboard
- tailor the Extension Adapter Dashboard for a specific page
- control the columns and filters

## Key Concept

An Extension Adapter Dashboard Content Sub Item does not replace the underlying Relational View. Instead, it provides a page-specific layer of configuration on top of that view.
...
The following settings are available for an Extension Adapter Dashboard Content Sub Item.
...
| Body Dashboard Name | Defines the inner dashboard that holds only the Extension Adapter Dashboard. | **This will need to be the name of the custom dashboard that holds an adapter based component.  See 'Create Extension Adapter Dashboard' below** |
...
| Auto Populate | Controls whether the Extension Adapter Dashboard automatically loads records when opened. | Not applicable for Extension Adapter Dashboards at this time. |
| Show Title Bar | Controls whether the title bar is shown. | |
| Show Filter Bar | Controls whether the filter bar is shown. | Not applicable to Extension Adapter Dashboards |
| Show Inline Entry | Controls whether the Enable **Inline Adding & Editing** option is available. | Not applicable for Extension Adapter Dashboards |

## Create Extension Adapter Dashboard

Create a custom dashboard that has an **adapter based component** on it, such as a BI Viewer component.  
Create it in a **custom maintenance unit** in the same Revfore Framework workspace as the Relational View.  

!!!Note.  Use the **XCP_xRfaConSubItm_AdptrDshbrds** maintenance unit or create a new one.  Just make sure it starts with **XCP_x**.  In it you will find a sample adapter and dashboard.
...
The **Column** tab is used to control which columns appear in the Extension Adapter Dashboard and how they behave.
...
The **Actions** tab does not apply to Extension Adapter Dashboards

## Dependents Tab

The **Dependents** tab is used to define how this Content Sub Item interacts with other Content Sub Items on the same page.

Extension Adapter Dashboard do not affect other Content Sub Items but other Content Sub Items, such as Dynamic Grids and List, can affect Extension Adapter Dashboards Content Sub Items.

## Typical Use Cases

Examples of Extension Adapter Dashboard Content Sub Item use cases include:

- using the same Relational View on multiple pages with different filters
- creating List that controls an Extension Adapter Dashboard that includes an interactive BI Viewer component on it
- showing different subsets of columns depending on the audience

## Design Guidance

When configuring an Extension Adapter Dashboard Content Sub Item:

- start with the underlying Relational View
- only expose the columns needed for the Extension Adapter Dashboard
- use page-level filters to reuse views without duplicating them
- keep the configuration focused on the intended use case

## Notes

- An Extension Adapter Dashboard Content Sub Item is a page-specific configuration layer on top of a Relational View.
- The same Relational View can be reused across multiple Content Sub Items and configured differently.
- Column, filter, and dependent settings allow each Extension Adapter Dashboard to be tailored for a specific page.
# List Content Sub Item

[← Back to Content Sub Items](index.md)

A **List** Content Sub Item is used to render a Relational View in a list experience within a Content Item layout.

List Content Sub Items allow you to tailor the presentation and behavior of a Relational View for a specific page. This means the same Relational View can be reused across multiple Content Sub Items and configured differently depending on the user experience you want to create.

## Overview

Use a List Content Sub Item to:

- render a Relational View in a single column list
- tailor the list for a specific page
...
A List Content Sub Item does not replace the underlying Relational View. Instead, it provides a page-specific layer of configuration on top of that view.
...
The following settings are available for a List Content Sub Item.
...
| Body Dashboard Name | Defines the inner dashboard that holds only the List. | This will auto populate from the content type but can be overridden with a custom dashboard. |
...
| Summary Font Size | Defines the font size for summary items. | Not applicable for Lists. |
...
| Show Inline Entry | Controls whether the **Add & Edit in Grid** option is available. | Not applicable for Lists |

## Column Tab

The **Column** tab is used to control which columns appear in the list and how they behave.
...
The **Actions** tab is used to control which actions are available for the List Content Sub Item.
...
This allows the system to understand how to filter the dependent Content Sub Item based on the selected record or records in the current list.

## Typical Use Cases

Examples of List Content Sub Item use cases include:

- using the same Relational View on multiple pages with different filters
- creating a selection list on one page and a pivot grid or BI Viewer dashboard on another
- showing different subsets of columns depending on the audience
- enabling different actions depending on the use case

## Design Guidance

When configuring a List Content Sub Item:

- start with the underlying Relational View
- only expose the columns needed for the list
- always include the primary key column in the included list but mark it as not visible
...
- A List Content Sub Item is a page-specific configuration layer on top of a Relational View.
- The same Relational View can be reused across multiple Content Sub Items and configured differently.
- Column, action, and dependent settings allow each list to be tailored for a specific page.
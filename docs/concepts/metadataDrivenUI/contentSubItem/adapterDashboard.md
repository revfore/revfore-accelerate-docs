# Adapter Dashboard Content Sub Item

[← Back to Content Sub Items](index.md)

An **Adapter Dashboard** Content Sub Item is used to render a Relational View in an Adapter Dashboard experience within a Content Item layout.

Adapter Dashboard Content Sub Items allow you to tailor the presentation and behavior of a Relational View for a specific page. This means the same Relational View can be reused across multiple Content Sub Items and configured differently depending on the user experience you want to create.

## Overview

Use an Adapter Dashboard Content Sub Item to:

- render a Relational View in an Adapter Dashboard
- tailor the Adapter Dashboard for a specific page
- control the columns and filters

## Key Concept

An Adapter Dashboard Content Sub Item does not replace the underlying Relational View. Instead, it provides a page-specific layer of configuration on top of that view.
...
The following settings are available for an Adapter Dashboard Content Sub Item.
...
| Body Dashboard Name | Defines the inner dashboard that holds only the Adapter Dashboard. | **This will need to be the name of the custom dashboard that holds an adapter based component.  See 'Create Adapter Dashboard' below** |
...
| Auto Populate | Controls whether the Adapter Dashboard automatically loads records when opened. | Not applicable for Adapter Dashboards at this time. |
| Show Title Bar | Controls whether the title bar is shown. | |
| Show Filter Bar | Controls whether the filter bar is shown. | Not applicable to Adapter Dashboards |
| Show Inline Entry | Controls whether the Enable **Inline Adding & Editing** option is available. | Not applicable for Adapter Dashboards |

## Create Adapter Dashboard
...
The **Column** tab is used to control which columns appear in the Adapter Dashboard and how they behave.
...
The **Actions** tab does not apply to Adapter Dashboards

## Dependents Tab

The **Dependents** tab is used to define how this Content Sub Item interacts with other Content Sub Items on the same page.

Adapter Dashboard do not affect other Content Sub Items but other Content Sub Items, such as Dynamic Grids and List, can affect Adapter Dashboards Content Sub Items.

## Typical Use Cases

Examples of Adapter Dashboard Content Sub Item use cases include:

- using the same Relational View on multiple pages with different filters
- creating List that controls an Adapter Dashboard that includes an interactive BI Viewer component on it
- showing different subsets of columns depending on the audience

## Design Guidance

When configuring an Adapter Dashboard Content Sub Item:

- start with the underlying Relational View
- only expose the columns needed for the Adapter Dashboard
- use page-level filters to reuse views without duplicating them
- keep the configuration focused on the intended use case

## Notes

- An Adapter Dashboard Content Sub Item is a page-specific configuration layer on top of a Relational View.
- The same Relational View can be reused across multiple Content Sub Items and configured differently.
- Column, filter, and dependent settings allow each Adapter Dashboard to be tailored for a specific page.
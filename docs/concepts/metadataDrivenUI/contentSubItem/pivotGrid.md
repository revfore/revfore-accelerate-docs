# Pivot Grid Content Sub Item

[← Back to Content Sub Items](index.md)

A **Pivot Grid** Content Sub Item is used to render a Relational View in a pivot grid experience within a Content Item layout.

Pivot Grid Content Sub Items allow you to tailor the presentation and behavior of a Relational View for a specific page. This means the same Relational View can be reused across multiple Content Sub Items and configured differently depending on the user experience you want to create.

## Overview

Use a Pivot Grid Content Sub Item to:

- render a Relational View in a pivot grid
- tailor the pivot grid for a specific page
- control layout, filters, and display settings

## Key Concept

A Pivot Grid Content Sub Item does not replace the underlying Relational View. Instead, it provides a page-specific layer of configuration on top of that view.
...
The following settings are available for a Pivot Grid Content Sub Item.
...
| Body Dashboard Name | Defines the inner dashboard that holds only the Pivot Grid. | This will auto populate from the content type but can be overridden with a custom dashboard. |
...
| Summary Font Size | Defines the font size for summary items. | Not applicable for Pivot Grids. |
| Auto Populate | Controls whether the pivot grid automatically loads records when opened. | Not applicable for Pivot Grids at this time. |
...
| Show Inline Entry | Controls whether the **Enable Inline Adding & Editing** option is available. | Not applicable for Pivot Grids |

## Column Tab

The **Column** tab is used to control which columns appear in the pivot grid and how they behave.
...
The **Actions** tab is used to control which actions are available for the Pivot Grid Content Sub Item.

This is presented as a multi-select checkbox list of all available actions.

The only action that is appropriate for a Pivot Grid is the Refresh action.
...
Pivot Grids do not affect other Content Sub Items but other Content Sub Items, such as Dynamic Grids and List, can affect Pivot Grid Content Sub Items.

## Typical Use Cases

Examples of Pivot Grid Content Sub Item use cases include:

- using the same Relational View on multiple pages with different filters
- creating a selection list on one page and a pivot grid on another
- showing different subsets of columns depending on the audience

## Design Guidance

When configuring a Pivot Grid Content Sub Item:

- start with the underlying Relational View
- only expose the columns needed for the pivot grid
- always include the primary key column in the included pivot grid but mark it as not visible
- use page-level filters to reuse views without duplicating them
- keep the configuration focused on the intended use case

## Notes

- A Pivot Grid Content Sub Item is a page-specific configuration layer on top of a Relational View.
- The same Relational View can be reused across multiple Content Sub Items and configured differently.
- Column, filter, and dependent settings allow each pivot grid to be tailored for a specific page.
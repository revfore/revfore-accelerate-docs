...
|---|---|---|---|
| Relational Model | int | Identifies the relational model associated with the view column. | This links the view column to the model that supports the view. |
...
| Column Name | nvarchar | Internal name of the view column. | No spaces or special characters are allowed. |
| Column Description | nvarchar | Description of the view column and its purpose. | Use this to document how the column should be used. |
...
| Sequence Number | int | Determines the relative order of the view column. | This is typically used to control display and processing order. |
...
| Is Enabled | bit | Indicates whether the view column is enabled for use. | Disabled columns are not intended for active use. |
| Is Visible | bit | Indicates whether the view column is visible to users. | Hidden columns may still be used for logic, filtering, or internal processing. |
| Model Allow Updates | bit | Indicates whether updates are allowed at the model column level. | Used to inherit or understand update behavior from the model. |
| Allow Updates | bit | Indicates whether users are allowed to update the view column. | Commonly used for editable data entry views. |
| Key Display Column | bit | Indicates whether the column should be treated as a key display column. | Useful when identifying the main descriptive field shown to users. |
...
| Group By Flag | int | Defines whether the column participates in group by behavior. | Used when the view supports grouped or aggregated output. |

...
- set the summary dimension members the unit's data rolls up to
...
| Summary Dimension Members | int | The members the unit's data rolls up to, one per dimension. | Same dimensions as the base members. Used where data is reported at a higher level than it is captured.
| Data Source Dimension | int | The dimension the data source summary members are selected from. | Identifies which dimension the members below belong to.
| Data Source Summary Members | int | The members used for each data source scope. | One each for All, Controlled, Controlled Itemized, Controlled Direct and Controlled Actual.
...
- Rolling a cost centre's detail up to a summary entity for reporting
...
- Base and summary members serve different purposes — base is where data is written, summary is where it is reported.

...
| Alias | nvarchar | Alternate short name or alias for the model source. | This is used in joins and expressions to distinguish sources.  Will auto-populate if a Table Relationship is specified, specifically from the 'Source Relational Table' 'Alias' and the 'Table Relationship' 'Join Alias Suffix'. No spaces or special characters are allowed. |
